...
Double_t jT0sigEl, jT0chi2, jT0dEdxSig, jT0beta, jT0dcaXY, jT0dca;
Double_t jT1sigEl, jT1chi2, jT1dEdxSig, jT1beta, jT1dcaXY, jT1dca;
...
Double_t jT0tofMass, jT1tofMass;
Double_t jDeltaDip, jRecPt, jRecY, jRecM;
Double_t jVtxX, jVtxY, jVtxZ, jVtxR;
Double_t jDcaPV, jVtxPVx, jVtxPVy, jVtxPVz;
Int_t jNselTrk;
Long64_t jEvtNum;
...
Short_t sign=0;
UShort_t minNhits=0;
Double_t maxAbsEta=kInf, maxNsigPID=kInf, maxTOFmass=kInf;
Double_t minTrkPt=-kInf;
Bool_t matchTof=0, projBemc=0;
Float_t maxAbsZvtx=kInf;
Float_t maxDca=kInf, maxDcaPV=kInf;
...
Bool_t isMC;
Double_t trackMass;
TH1I *hEvtCount, *hTrkCount;
enum evtCnt {kAnaL=1, kTrg, kMaxTrk, kPair, kVtx, kDca, kPID, kTofM, kMaxM, kDip, kSign, kDcaPV, kZvtx, kMaxCnt};
...
Bool_t selectPhoton(StUPCTrack *gam);
Bool_t selectPair(const StUPCTrack *trk0, const StUPCTrack *trk1);
inline Bool_t selectTrack(const StUPCTrack *trk);
Double_t getTOFmass(const StUPCTrack *trk);
StUPCTrack *getPhoton(const StUPCTrack *trk0);
void FillRecTree(const StUPCTrack *gam, const StUPCTrack *trk0, const StUPCTrack *trk1);
...
int main(void) {
  ...
  //string in = "trees/convel/test0/StUPC_conv_v5.root";
  //string in = "/home/jaroslav/analyza/StUPCLib/ver1/ana/muDst_run1_prod.txt";
  string in = "trees/test/muDst_test1/StUPC_muDst_test1_all.root";
  //string in = "trees/muDst_sec0/StUPC_muDst_sec0_all.root";
  ...
  maxAbsZvtx = 100.;    // maximal Z position of secondary vertex, absolute value
  maxDca = 0.1;          // maximal DCA to secondary vertex
  maxDcaPV = 0.2;        // photon maximal DCA to primary vertex
...
void RunTracks() {
  ...
  for(Int_t i=0; i<ntrk; i++) {
    ...
    for(Int_t j=i+1; j<ntrk; j++) {
      ...
      //pair from secondary vertex
      if( !selectPair(trk0, trk1) ) continue;
      //pair selected

      //photon candidate
      StUPCTrack *gam = getPhoton(trk0);
      if( !selectPhoton(gam) ) continue;
      //photon selected

      FillRecTree(gam, trk0, trk1);
    }
  }//pairs loop
...
Bool_t selectPhoton(StUPCTrack *gam) {
  ...
  if(!gam) return kFALSE;

  //maximal DCA to primary vertex
  if( gam->getDcaZ() > maxDcaPV ) return kFALSE;
  hEvtCount->Fill( kDcaPV );

  //z position of primary vertex
  if( TMath::Abs(gam->getVertex()->getPosZ()) > maxAbsZvtx ) return kFALSE;
...
Bool_t selectPair(const StUPCTrack *trk0, const StUPCTrack *trk1) {
  ...
  if( trk0->getDcaZ() > maxDca || trk1->getDcaZ() > maxDca ) return kFALSE;
  hEvtCount->Fill( kDca );

  //moved to selectPhoton to select z of primary vertex
  ...
  //TOF mass of tracks
  Double_t tmass0 = getTOFmass(trk0);
  Double_t tmass1 = getTOFmass(trk1);
  if( tmass0*tmass0 + tmass1*tmass1 > maxTOFmass*maxTOFmass ) return kFALSE;
  hEvtCount->Fill( kTofM );
...
StUPCTrack *getPhoton(const StUPCTrack *trk0) {
  ...
  //get photon candidate corresponding to selected pair of tracks

  //track secondary vertex
  UInt_t secID = trk0->getVertexId();

  //photon candidate has sec vtx id set as setBemcClusterId
  for(Int_t itrk=0; itrk<upcEvt->getNumberOfTracks(); itrk++) {
    StUPCTrack *trk = upcEvt->getTrack(itrk);

    if( trk->getCharge() != 0 ) continue;

    if( trk->getBemcClusterId() == secID ) return trk;
  }
...
void FillRecTree(const StUPCTrack *gam, const StUPCTrack *trk0, const StUPCTrack *trk1) {
  ...
  //TOF mass
  jT0tofMass = getTOFmass(trk0);
  jT1tofMass = getTOFmass(trk1);
  ...
  //secondary vertex position, both tracks come from the same vertex at this point
  StUPCVertex *vtx = trk0->getVertex();
  jVtxX = vtx->getPosX();
  jVtxY = vtx->getPosY();
  jVtxZ = vtx->getPosZ();
  //radial position of secondary vertex
  jVtxR = TMath::Sqrt(jVtxX*jVtxX + jVtxY*jVtxY);

  //photon candidate DCA to primary vertex
  jDcaPV = gam->getDcaZ();
  //position of primary vertex
  StUPCVertex *pv = gam->getVertex();
  jVtxPVx = pv->getPosX();
  jVtxPVy = pv->getPosY();
  jVtxPVz = pv->getPosZ();
...
TFile *CreateOutputTree(const string& out) {
  ...
  jRecTree ->Branch("jT0dca", &jT0dca, "jT0dca/D");
  ...
  jRecTree ->Branch("jT1dca", &jT1dca, "jT1dca/D");
  ...
  jRecTree ->Branch("jT0tofMass", &jT0tofMass, "jT0tofMass/D");
  jRecTree ->Branch("jT1tofMass", &jT1tofMass, "jT1tofMass/D");
  ...
  jRecTree ->Branch("jVtxR", &jVtxR, "jVtxR/D");
  jRecTree ->Branch("jDcaPV", &jDcaPV, "jDcaPV/D");
  jRecTree ->Branch("jVtxPVx", &jVtxPVx, "jVtxPVx/D");
  jRecTree ->Branch("jVtxPVy", &jVtxPVy, "jVtxPVy/D");
  jRecTree ->Branch("jVtxPVz", &jVtxPVz, "jVtxPVz/D");
  ...
  jRecTree ->Branch("maxTOFmass", &maxTOFmass, "maxTOFmass/D");
  ...
  jRecTree ->Branch("maxDca", &maxDca, "maxDca/F");
  jRecTree ->Branch("maxDcaPV", &maxDcaPV, "maxDcaPV/F");
...
void PrintStat(ostream& out, const string& innam, const string& outnam, Int_t lmg) {
  ...
  //analysis counter
  //enum evtCnt {kAnaL=1, kTrg, kMaxTrk, kPair, kVtx, kDca, kPID, kMaxM, kDip, kSign, kZvtx, kGam, kMaxCnt};
  //Int_t wdt=15;
  st << "------------------------" << endl;
  st.precision(0);
  ...
  statMsg(st, "DCA: ", kDca);
  statMsg(st, "PID: ", kPID);
  statMsg(st, "TOF mass: ", kTofM);
  statMsg(st, "Max mass: ", kMaxM);
  statMsg(st, "Delta dip: ", kDip);
  statMsg(st, "Sign: ", kSign);
  statMsg(st, "DCA PV: ", kDcaPV);
  statMsg(st, "PV z_vtx: ", kZvtx);
  st << "------------------------" << endl;
